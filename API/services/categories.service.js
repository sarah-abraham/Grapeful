...
async function createCategory(params, callback) {
  // If category name is not provided
...
async function getCategories(params, callback) {
  ...
  let perPage = Math.abs(params.pageSize) || MONGO_DB_CONFIG.pageSize; // Pagination
  let page = (Math.abs(params.page) || 1) - 1; // index number of the page to be displayed
...
async function getCategoryById(params, callback) {
  ...
  category
    .findById(categoryId)
    .then((response) => {
      if (!response) callback("Not Found Catgeory with Id " + categoryId);
